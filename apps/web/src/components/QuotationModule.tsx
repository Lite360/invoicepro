...
export const QuotationModule: React.FC<QuotationModuleProps> = ({ company, onPreview, onInvoiceCreated }) => {
  ...
  const handleSave = async (status: string = 'Pending') => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.id) onPreview('Quotation', data);
    } catch (e) {
  ...
  const handleConvertToInvoice = async () => {
    if (!formData.id) {
      ...
      if (data.id) {
        ...
        if (convData.invoice) onInvoiceCreated();
      }
    } else {
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-6 mb-8 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3 w-full md:w-auto">
            ...
            <button type="button" onClick={() => onPreview('Quotation', formData)}
            ...
            <button type="button" onClick={() => handleSave('Issued')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 bg-slate-50 p-6 rounded-2xl border border-slate-100">
          ...
          <div>
            ...
            <input type="text" value={formData.projectTitle || ''}
        ...
        <div className="space-y-4 mb-8">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={addItem}
              className="flex items-center space-x-1.5 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold rounded-lg transition">
